...
void MenuState::draw(cgf::Game* game) {
    ...
    if (!didOrganizeMenuButtons) {
        organizeMenuButtons(screen->getSize());
...
void MenuState::organizeMenuButtons(Vector2u windowSize) {

    float width = 250;
    float height = 100;
    float spacing = 30;

    float y = windowSize.y/2 - (menuButtons.size()*height + (menuButtons.size()-1)*spacing)/2;
    ...
    for (auto &button : menuButtons) {
        ...
        button.text.setPosition(windowSize.x/2 - textBounds.width/2, y + height/2 - textBounds.height/2 - 10);
